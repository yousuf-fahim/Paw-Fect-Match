...
export const useSignup = () => {
  ...
  const signup = async (name, email, password, otp) => {
    ...
    try {
      const otpResponse = await fetch('https://pawfect-match-b8jp.onrender.com/api/verifyotp', {
        method: 'POST',
      ...
      if (!otpResponse.ok) {
        ...
        const response = await fetch('https://pawfect-match-b8jp.onrender.com/signup', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
